...
import {
  useGradosEducacion,
  useCreateGradoEducacion,
  useUpdateGradoEducacion,
  useDeleteGradoEducacion,
} from '../../hooks/useGradosEducacion';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-xl shadow-lg w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h2 className="text-base font-semibold text-gray-800">Nuevo grado</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-2xl leading-none">&times;</button>
        </div>
        <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-5 space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Nombre del grado</label>
            <input {...register('nombre')} className={inputCls} placeholder="Ej: Primero de Primaria" autoFocus />
...
function ModalEditar({ grado, onClose, onSubmit, isPending }: ModalEditarProps) {
  const { register, handleSubmit, formState: { errors } } = useForm<FormValues>({
    resolver: zodResolver(schema),
    defaultValues: { nombre: grado.nombre },
  });
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-xl shadow-lg w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h2 className="text-base font-semibold text-gray-800">Editar grado</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-2xl leading-none">&times;</button>
        </div>
        <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-5 space-y-4">
          <div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-lg w-full max-w-sm p-6">
        ...
        <p className="text-sm text-gray-600 mb-6">
          ¿Estás seguro de que deseas eliminar el grado <span className="font-medium">{nombre}</span>?
...
export default function GradosEducacionAdminPage() {
  const [showCrear, setShowCrear] = useState(false);
  const [editando, setEditando] = useState<GradoEducacion | null>(null);
  const [eliminandoId, setEliminandoId] = useState<string | null>(null);

  const { data: grados, isLoading, isError } = useGradosEducacion();
  const createMutation = useCreateGradoEducacion();
  const updateMutation = useUpdateGradoEducacion();
  const deleteMutation = useDeleteGradoEducacion();

  const gradoEliminando = grados?.find((g) => g.id === eliminandoId);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Grados de Educación</h1>
          <p className="text-sm text-gray-500 mt-0.5">Gestión de grados educativos del sistema.</p>
        </div>
        <button
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-x-auto">
        {isError ? (
          <div className="p-8 text-center text-sm text-red-500">Error al cargar los grados.</div>
        ) : (
              ...
              {isLoading ? (
                ...
                grados.map((g) => (
                  <tr key={g.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-gray-900">{g.nombre}</td>
      ...
      {eliminandoId && gradoEliminando && (
        <ModalConfirmar
          nombre={gradoEliminando.nombre}
